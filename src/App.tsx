  ...
  return (
    <main className="app-shell">
      ...
      <footer className="player-bar">
        ...
        <div className="now-playing"><img src={current?.cover || DEFAULT_COVER} alt="" /><div><strong>{current?.name || "还没有播放歌曲"}</strong><small>{current ? `${current.singer} · ${current.album}` : "选择一首歌曲开始聆听"}</small></div><div className="now-playing-actions"><button className={isCurrentFavorite ? "liked" : ""} onClick={() => current && toggleFavorite(current)} disabled={!current} aria-label="收藏当前歌曲" title="收藏"><Heart size={18} fill={isCurrentFavorite ? "currentColor" : "none"} /></button><button onClick={() => current && setTrackToAdd(current)} disabled={!current} aria-label="将当前歌曲添加到歌单" title="添加到歌单"><Plus size={18} /></button></div></div>
